...
├── mcp-config.json      # Конфигурация MCP серверов (пример)
├── run_duckdb_mcp.py    # Wrapper для запуска DuckDB MCP
├── start_duckdb_mcp.ps1 # Скрипт запуска DuckDB MCP сервера
...
- `watch-logs.bat` - просмотр логов бота в реальном времени
- `open-docker.ps1` - открытие/восстановление окна Docker Desktop
